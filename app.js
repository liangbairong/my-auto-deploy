...
async function main() {
  try {
    ...
    SELECT_CONFIG.openCompress ? await compressFile(SELECT_CONFIG.targetDir, localFile, SELECT_CONFIG.projectType) : '' //压缩
    ...
    if(SELECT_CONFIG.command){
      await runCommand(sshServer.ssh, SELECT_CONFIG.command, SELECT_CONFIG.deployDir + SELECT_CONFIG.releaseDir) // 执行脚本
    }
    console.log('部署完毕！')
  } catch (err) {
