...
function compressFile (targetDir, localFile, type) {
  return new Promise((resolve, reject)=>{
    ...
    if(type==='node'){
      const dirArray = fs.readdirSync(targetDir);
      dirArray.forEach(item=>{
        if(item!='node_modules'){
          if(item.split('.').length===1){
            archive.directory(targetDir+'/'+item, 'dist/'+item)
          }else{
            archive.file(targetDir+'/'+item, { name: item, prefix:'dist/' });
          }
        }
      })
    }else{
